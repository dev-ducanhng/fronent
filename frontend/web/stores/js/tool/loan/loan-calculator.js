...
$(function() {
	...
	$('body').on('click','.calculator-info .accordion-title',function()
	{
	...
	var implementFixedMonthlyPayment = function($wraper, data_cal) {
		...
		var borrowAmount = parseFloat($wraper.find('.borrow-amount').attr('data-value'));
	...
	var implementBorrow = function($wraper, data) {
		...
		var calculatorBorrow = function(income, expenses, termBorrow, interestBorrow) {
			...
			var result = (0.8 * (income - expenses)) / (1/termMonth + interestMonth);
	...
	*/
	...
	function getYearLabel(year, isYear) {
		...
		if (lang === 'en') {
			...
			return singular + ' ' + language.ordinalSeparator + ' ' + yearText;
		}
	}
