...
<script language="JavaScript" type="text/javascript">
    $(document).ready(function() {
        ...
        $('.btn_see_more').click(function() {
            ...
            $.ajax({
                url: 'tim-chi-nhanh-cong-ty-bao-hiem.html',
                data: {
                    page_size: page_size
                },
                type: 'POST',
                success: function(res) {
                    ...
                    var rendered = '';
                    $.each(jsonData['data_insurance'], function(key, val) {
        ...
        $('#btn_submit').on('click', function(e) {
            ...
            if (company == '') {
                ...
                if (province == '') {
                    var URL = encodeURI("cong-cu/tim-chi-nhanh-cong-ty-bao-hiem/" + company_name + '-' + company) + '.html';
                } else {
                    if (district == '') {
                        var URL = encodeURI("cong-cu/tim-chi-nhanh-cong-ty-bao-hiem/" + company_name + '-' + province_name + '-' + company + '-' + province) + '.html';
                    } else {
                        var URL = encodeURI("cong-cu/tim-chi-nhanh-cong-ty-bao-hiem/" + company_name + '-' + district_name + '-' + province_name + '-' + company + '-' + province + '-' + district + ".html");
                    }
                }
            }
            if (company == '' && province != '') {
                $('.simply-toast').fadeOut();
                $.simplyToast('warning', "Bạn muốn tìm chi nhánh của công ty bảo hiểm nào?");
            } else if (company == '' && province == '') {
                $('.simply-toast').fadeOut();
                $.simplyToast('warning', "Bạn muốn tìm chi nhánh của công ty Bảo hiểm, Tỉnh/Thành Phố nào?");
            } else {
...
<div class="container">
    <div class="breadcrumbs">
        ...
        <?php
        use yii\helpers\Url;
        if ($_SERVER['REQUEST_URI'] == Url::to(['tool/find-insurance'])) : ?>
            <span class="span_pc">Tìm chi nhánh công ty bảo hiểm</span>
            <span class="span_mobile">Tìm chi nhánh...</span>
        <?php else : ?>
            <a href="/cong-cu/tim-chi-nhanh-cong-ty-bao-hiem.html" class="span_pc">Tìm chi nhánh công ty bảo hiểm</a>
            <a href="/cong-cu/tim-chi-nhanh-cong-ty-bao-hiem.html" class="span_mobile">Tìm chi nhánh...</a>
        <?php endif; ?>
    </div>
    <div class="filter">
        <div class="title">
            <?php if (!empty($_GET['company']) & empty($_GET['province'])) : ?>
                <h1>DANH SÁCH VĂN PHÒNG, TỔNG ĐẠI LÝ CÁC CÔNG TY BẢO HIỂM NHÂN THỌ <?= mb_strtoupper($array_company[$_GET['company']], 'UTF-8') ?></h1>
            <?php elseif (!empty($_GET['company']) & !empty($_GET['province']) & empty($_GET['district'])) : ?>
                <h1>CHI NHÁNH, VĂN PHÒNG, TỔNG ĐẠI LÝ CÔNG TY BẢO HIỂM NHÂN THỌ <?= mb_strtoupper($array_company[$_GET['company']], 'UTF-8') ?> TẠI <?= mb_strtoupper($array_province[$_GET['province']], 'UTF-8') ?></h1>
            <?php elseif (!empty($_GET['company']) & !empty($_GET['province']) & !empty($_GET['district'])) : ?>
                <h1>CHI NHÁNH, VĂN PHÒNG, TỔNG ĐẠI LÝ CÔNG TY BẢO HIỂM NHÂN THỌ <?= mb_strtoupper($array_company[$_GET['company']], 'UTF-8') ?> TẠI <?= mb_strtoupper($array_district[$_GET['district']], 'UTF-8') ?> - <?= mb_strtoupper($array_province[$_GET['province']], 'UTF-8') ?></h1>
            <?php else : ?>
                <h1>DANH SÁCH VĂN PHÒNG, TỔNG ĐẠI LÝ CÁC CÔNG TY BẢO HIỂM NHÂN THỌ TRÊN TOÀN QUỐC</h1>
            <?php endif; ?>
        </div>
        <div class="description">
            <?php if (!empty($_GET['company']) & empty($_GET['province'])) : ?>
                <h2>Cập nhật danh sách địa chỉ chi nhánh, phòng giao dịch của <?= $array_company[$_GET['company']] ?> trên toàn quốc, giúp bạn tiết kiệm tối đa thời gian di chuyển.</h2>
            <?php elseif (!empty($_GET['company']) & !empty($_GET['province']) & empty($_GET['district'])) : ?>
                <h2>Cập nhật thông tin địa chỉ, số điện thoại Chi nhánh, Văn phòng, Tổng đại lý công ty bảo hiểm nhân thọ <?= $array_company[$_GET['company']] ?> tại <?= $array_province[$_GET['province']] ?>, giúp bạn an tâm tìm hiểu và lựa chọn.</h2>
            <?php elseif (!empty($_GET['company']) & !empty($_GET['province']) & !empty($_GET['district'])) : ?>
                <h2>Cập nhật thông tin địa chỉ, số điện thoại Chi nhánh, Văn phòng, Tổng đại lý công ty bảo hiểm nhân thọ <?= $array_company[$_GET['company']] ?> tại <?= $array_district[$_GET['district']] ?> <?= $array_province[$_GET['province']] ?>, giúp bạn an tâm tìm hiểu và lựa chọn.</h2>
            <?php else : ?>
                <h2>Cập nhật danh sách địa chỉ chi nhánh, phòng giao dịch của các công ty bảo hiểm trên toàn quốc, giúp bạn tiết kiệm tối đa thời gian di chuyển.</h2>
            <?php endif; ?>
    ...
    <div class="list">
        <div class="row content_row">
            <div class="left_content <?php if (empty($_GET['province'])) echo "col-12 col-lg-8" ?>" style="padding-left: 15px;padding-right: 15px;width: 100%;">
                <?php if (!empty($_GET['company']) & empty($_GET['province'])) : ?>
                    <div id="template_data_province">
                        <div class="title_province">CHI NHÁNH, PHÒNG GIAO DỊCH CÔNG TY BẢO HIỂM <?= mb_strtoupper($array_company[$_GET['company']], 'UTF-8') ?> CÁC TỈNH THÀNH</div>
                ...
                <?php elseif (!empty($_GET['company']) & !empty($_GET['province'])) : ?>
                    <div class="list_atm" <?php if ($count_record == 1) echo "style='padding-bottom:80px;'"; elseif ($count_record == 2) echo "style='padding-bottom:20px;'"; ?>>
                        <table class="table">
                ...
                <?php else : ?>
                    <div id="template_data_insurance">
                        <?php foreach ($data_insurance as $item) : ?>
            ...
            <?php else : ?>
                <div class="right_content col-12 col-lg-4">
                    <div class="right_btn">
                        ...
                        <a href="https://thebank.vn/danh-ba-ngan-hang.html" class="btn_2" target="_blank">Thông tin ngân hàng</a>
                        ...
                        <a href="https://thebank.vn/cong-cu/tim-chi-nhanh-ngan-hang.html" class="btn_5" target="_blank">Chi nhánh ngân hàng</a>
                        <div class="banner">
